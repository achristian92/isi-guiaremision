...
use App\Requests\GuiaItemRequest;
use App\Requests\GuiaRequest;
use App\Requests\GuiaTransporteRequest;
use App\Voucher\Order\Despatches\Despatch;
use App\Voucher\Order\Orders\Order;
use Carbon\Carbon;
...
class GuiaRemisionController extends Controller
{
    ...
    {
        return view('guiaremision.index',[
            'guias' => GuiaRemision::orderBy('delivery_date')->get()
        ]);
    }

    public function create()
    {
        return view('guiaremision.create',[
    ...
    public function store(GuiaRequest $request)
    {
        ...
        $guia->sequence_id        = $seq->id;
        $guia->document_prefix     = $seq->prefix;
        $guia->document_id        = $doc_id;
        $guia->document_reference = $seq->getDocumentReference($doc_id);
        $guia->save();

        return redirect()->route('guiaremision.show',$guia->id)->with('message',"Registro creado");
    }

    public function show(int  $id)
    {
        return view('guiaremision.show',[
    ...
    public function pdf(int $id)
    {
        ...
        if ( $guia->items->count() === 0 )
            return redirect()->back()->with('error', 'No tienes Produtos agregados');

        if ($guia->modality_id == '1')
            if (empty($guia->trans_ruc_pub) || empty($guia->trans_razon_name_pub))
                return redirect()->back()->with('error', 'Completa la información de transporte');

        if ($guia->modality_id == '2')
            if (empty($guia->trans_placa_pri) || empty($guia->trans_nro_doc_priv))
                return redirect()->back()->with('error', 'Completa la información de transporte');
    ...
    public function item(GuiaItemRequest $request, int $id)
    {
        ...
        return redirect()->route('guiaremision.show',$id)->with('message',"Registro creado");
    }

    public function transporte(GuiaTransporteRequest $request, int $id)
    {
        $guia = GuiaRemision::find($id);
        $guia->update($request->all());

        return redirect()->route('guiaremision.show',$id)->with('message',"Registro actualizado");
    }

    public function sunat(int $id)
    {
        $guia = GuiaRemision::find($id);
      ...
      <contentFile>'.$contenido_del_zip.'</contentFile>
        ...
        if($httpcode == 200){
            ...
            if(isset($doc->getElementsByTagName('applicationResponse')->item(0)->nodeValue)){
                ...
                $guia->estado_sunat = 1;
                $guia->name_xml = '/xml/'.$nombrexml.'.XML';
                $guia->src_cdr = $rutacdr;
                $guia->save();

                return redirect()->route('guiaremision.show',$guia->id)->with('message',"Guía válido, ahora puedes descargar el xml y el cdr");

            }else{
                $codigo = $doc->getElementsByTagName("faultcode")->item(0)->nodeValue;
                $mensaje = $doc->getElementsByTagName("faultstring")->item(0)->nodeValue;
                return redirect()->back()->with('error', "error ".$codigo.": ".$mensaje);
            }
        }else{
            return redirect()->back()->with('error', "Problema de conexión");
        }
        curl_close($ch);
    }
}
